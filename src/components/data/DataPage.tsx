...
const DataPage: React.FC = () => {
  ...
  const {
    ...
    editingDataId
      ? { id: editingDataId, projectId: selectedProjectId || undefined }
      : skipToken,
  ...
  const handleUpdateSubmit = async (formData: any) => {
    ...
    try {
      ...
      if (
        errorDetail === "Vault is sealed. Cannot update data value." ||
        errorDetail ===
          "No master key available. Vault may need to be unsealed." ||
        err?.status === 503
      ) {
        setIsEditModalOpen(false);
        setEditingData(null);
  ...
  const handleManualRotate = async () => {
    ...
    try {
      ...
      const errorDetail = err?.data?.detail;
      setError(
        errorDetail || t("secrets.rotationError", "Failed to rotate secret"),
      );

      // If vault is sealed, close the modal
      if (
        errorDetail === "Vault is sealed. Please unseal the vault first." ||
        errorDetail ===
          "No master key available. Vault may need to be unsealed." ||
        err?.status === 503
      ) {
      ...
      <div className="space-y-6">
        <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 mb-6">
          ...
          <select
            ...
            disabled={
              !fullData ||
              !user ||
              String(user.user_id) !== String(fullData.user_id)
            }
        ...
        <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            {(editingData.data_type === "credentials" ||
              editingData.data_type === "api_key") && (
              <button
                type="button"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditRotationInterval(parseInt(e.target.value) || 0)
                }
              ...
              <p className="text-xs text-gray-500 mt-1">
                {t(
                  "secrets.rotationHelp",
                  "Set to 0 to disable automatic rotation reminders.",
                )}
              </p>
            </div>
            {fullData.next_rotation_date && (
              <div>
                ...
                <div className="px-3 py-2 bg-white dark:bg-gray-600 rounded-md border border-gray-200 dark:border-gray-500 text-sm">
                  {formatDate(fullData.next_rotation_date)}
                </div>
              </div>
            )}
          </div>
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() =>
              navigate(
                selectedProjectId
                  ? `/dashboard/create-data?projectId=${selectedProjectId}`
      ...
      {dataList && dataList.length > 0 ? (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-gray-600 text-sm mb-3">
                    {data.description ||
                      t("secrets.noDescription", "No description")}
                  </p>

                  <div className="flex items-center gap-4 mt-3 text-xs text-gray-500">
                    <span>
                      {t("secrets.version")}: {data.version ?? "N/A"}
                    </span>
                    <span>•</span>
                    <span>
                      {t("secrets.created")}: {formatDate(data.created_at)}
                    </span>
                  </div>
                </div>
        ...
        <div className="card text-center py-12">
          ...
          <p className="text-gray-600 mb-4">{t("secrets.getStarted")}</p>
          <button
      ...
      >
        {fullData?.decrypt_error && (
          <Alert
            type="error"
            message={`${t("secrets.decryptError", "Decryption error")}: ${fullData.decrypt_error}`}
          />
        )}
        {isLoadingFullData ? (
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3">
            ...
            <button onClick={handleManualRotate} className="btn-primary">
