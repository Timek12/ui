...
export const ProjectDetails: React.FC<ProjectDetailsProps> = ({
  ...
  const { data: secrets, isLoading: isLoadingSecrets } = useGetDataQuery({
    projectId,
  });
  ...
  if (isLoadingProject || isLoadingMembers || isLoadingSecrets)
    return <LoadingSpinner />;
  if (!project) return <Alert type="error" message={t("projects.notFound")} />;

  const currentUserMember = members?.find(
    (m) => String(m.user_id) === String(user?.user_id),
  );
  ...
  return (
    <div className="space-y-8">
              ...
              >
                <div>
                  <p className="font-medium text-gray-900 dark:text-white">
                    {t("projects.userId")}: {member.user_id}
                  </p>
      ...
      >
        {addMemberError && (
          <Alert type="error" message={addMemberError} className="mb-4" />
        )}
