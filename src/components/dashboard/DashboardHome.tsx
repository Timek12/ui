...
const DashboardHome: React.FC = () => {
  ...
  const { data: vaultStatus, isLoading: vaultLoading } = useGetVaultStatusQuery(
    undefined,
    { skip: !isAdmin },
  );
  ...
  const { data: auditData, isLoading: auditLoading } = useGetLogsQuery({
    limit: 5,
    offset: 0,
  });
      ...
      ? [
          {
            ...
            value: vaultStatus?.vault.sealed
              ? t("dashboard.sealed")
              : t("dashboard.unsealed"),
            icon: Lock,
            color: vaultStatus?.vault.sealed
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          ...
          >
            <div
              className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${stat.color} opacity-10 rounded-bl-full -mr-4 -mt-4 transition-opacity group-hover:opacity-20`}
            />
            ...
            <div className="space-y-4">
              {auditData?.logs && auditData.logs.length > 0 ? (
                auditData.logs.map((log) => (
                  <div
                    key={log.id}
                    className="flex items-center justify-between p-4 bg-gray-50/50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors"
                  >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white">
                    {t("dashboard.addNewSecret")}
                  </h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {t("dashboard.storeCredential")}
                  </p>
                </div>
              </div>
            </button>
