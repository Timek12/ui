...
const AuditDashboard: React.FC = () => {
  ...
  if (error)
    return <div className="p-4 text-red-500">{t("audit.loadError")}</div>;
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-700">
                      ...
                      <th className="px-4 py-2 text-right dark:text-gray-300">
                        {t("audit.count")}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {stats.ipData.map((ip, idx) => (
                      <tr key={idx} className="border-b dark:border-gray-700">
                        ...
                        <td className="px-4 py-2 text-right font-medium dark:text-gray-300">
                          {ip.value}
                        </td>
                      </tr>
                    ))}
      ...
      {stats && stats.recentFailures.length > 0 && (
        <div className="bg-red-50 dark:bg-red-900/10 p-4 rounded-lg shadow border border-red-200 dark:border-red-900/30">
          <h3 className="text-lg font-semibold text-red-800 dark:text-red-400 mb-2">
            {t("audit.recentFailures")}
          </h3>
          <div className="space-y-2">
            {stats.recentFailures.map((log: any) => (
              <div
                key={log.id}
                className="flex justify-between items-center bg-white dark:bg-gray-800 p-2 rounded border border-red-100 dark:border-red-900/20"
              >
                <div>
      ...
      <div className="overflow-x-auto bg-white dark:bg-gray-800 shadow-md rounded-lg">
        <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center">
          <h3 className="text-lg font-semibold dark:text-white">
            {t("audit.detailedLogs")}
          </h3>
          <span className="text-sm text-gray-500 dark:text-gray-400">
            {t("audit.showing")} {tablePage * logsPerTablePage + 1}-
            {Math.min((tablePage + 1) * logsPerTablePage, logs.length)}{" "}
            {t("audit.of")} {logs.length}
          </span>
        </div>
        <table className="min-w-full leading-normal">
          ...
          <tbody>
            {logs
              .slice(
                tablePage * logsPerTablePage,
                (tablePage + 1) * logsPerTablePage,
              )
              .map((log) => (
                <tr key={log.id}>
            ...
            {logs.length === 0 && (
              <tr>
                <td
                  colSpan={6}
                  className="px-5 py-5 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-center dark:text-gray-400"
                >
                  {t("audit.noLogs")}
                </td>
              </tr>
        ...
        <div className="px-4 py-3 border-t dark:border-gray-700 flex justify-between items-center">
          ...
          <span className="text-sm dark:text-white">
            {t("audit.page")} {tablePage + 1} {t("audit.of")}{" "}
            {Math.ceil(logs.length / logsPerTablePage)}
          </span>
          <button
